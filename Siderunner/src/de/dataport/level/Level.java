...
public class Level
{
	...
	public List<Gameblock> level1()
	{
		...
		Gameblock rect1 = new Gameblock(200, Main.test.getY() - 40, 60, 40,
				false, "Peter", Color.BLUE);
		content.add(rect1);
		Gameblock rect2 = new Gameblock(400, Main.test.getY() - 20, 100, 10,
				false, "Peter", Color.BLUE);
		content.add(rect2);
	...
	{
		Rectangle rec = new Rectangle(gameblock.getX(), gameblock.getY(),
				gameblock.getWidth(), gameblock.getHeigth());
		for (Gameblock gb : content)
		{
			if (!(gameblock.hashCode() == gb.hashCode()))
			{
				Rectangle rgb = new Rectangle(gb.getX(), gb.getY(),
						gb.getWidth(), gb.getHeigth());
