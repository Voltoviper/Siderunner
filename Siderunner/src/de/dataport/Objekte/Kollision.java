	...
	public static int[] kollision_rechts(Spielfigur spielfigur, Level level)
	{
		...
		for (Gameblock rect : level.getListe())
		{
			if (rect != null)
			{
				...
				if ((Main.spieler.getX() + Spielfigur.getBreite() + Spielfigur.getGeschwindigkeit() > rect.getX())
				...
				else if ((Main.spieler.getX() - (2 * Spielfigur.getGeschwindigkeit()) < rect.getX()) && (Main.spieler.getX() > rect.getX()))
				{
					if ((Main.spieler.getY() - (2 * 50) < rect.getY() + rect.getHeigth() && Main.spieler.getY() > rect.getY())
							|| Main.spieler.getY() + Spielfigur.getHoehe() + (2 * 50) > rect.getY()
							&& Main.spieler.getY() + Spielfigur.getHoehe() < rect.getY())
		...
		for (Gameblock rect : level.getListe())
		{
			if (rect != null)
			{
				if (figur.getY() + Main.spieler.getHeigth() + 50 > rect.getY()
						&& (figur.getX() + Main.spieler.getWidth() > rect.getX() && figur.getX() < rect.getX() + rect.getWidth()))
				{
	...
	private static int[] auswählen(ArrayList<Gameblock> beruehrpunkte2, int Orientierung)
	{
		...
		case 0:
			for (Gameblock g : beruehrpunkte2)
			{
				...
				if ((g.getY() + g.getHeigth() > Main.spieler.getY()) && (g.getY() < Main.spieler.getY() + Main.spieler.getHeigth())
		...
		for (Gameblock g : unten)
		{
			...
			{
				if ((g.getX() + g.getWidth() > Main.spieler.getX()) && (g.getY() >= Main.spieler.getY() + Main.spieler.getHeigth())
						&& (g.getX() < Main.spieler.getX() + Spielfigur.getBreite()))
				{
